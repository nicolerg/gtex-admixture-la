...
def parse_args():
  ...
  parser.add_argument('--snp_map', help='headerless, 3-column file with 1 row per SNP: chromosome, bp, cM', required=True)
  ...
  parser.add_argument('--chr', help='name of single chromosome to process', default='', required=False)
...
def find_haplotype_bounds(index, add, pop_order, hap,npop):
  for chr in chrs:
    ...
    rfmix = gzip.open(re.sub(r'chr[X0-9]+', 'chr' + str(chr), args.rfmix))
    ...
    snp_locations = gzip.open(snp_file)
    snp_map = gzip.open(re.sub(r'chr[X0-9]+', 'chr' + str(chr), args.snp_map))
    # snp_map = gzip.open('/mnt/lab_data/montgomery/nicolerg/rfmix-admixed/inputs/snp-gd-bed-chr' + str(chr) + '.gz')
    ...
    for line in rfmix:
      ...
      if counter == 1:
        last_anc_pos_cm = [myLine[2*index + add], my_map[1], my_pos]
        post_anc_pos_cm = [myLine[2*index + add], my_map[1], my_pos]
        continue
      
      #start regular iterations
      current_anc_pos_cm = [myLine[2*index + add], my_map[1], my_pos]
