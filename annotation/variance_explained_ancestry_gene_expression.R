...
expression = fread(cmd=sprintf('zcat GTEx_Analysis_2017-06-05_v8/eqtl/GTEx_Analysis_v8_eQTL_expression_matrices/%s.v8.normalized_expression.bed.gz',tissue), sep='\t', header=T)
tss = fread(cmd='zcat tss/hg19_tss_localcov.tsv.gz', sep='\t', header=T) # local ancestry for hg19 TSSs. see interpolate-tss-la.py
gpc = fread('GTEx_Analysis_2017-06-05_v8/genotypes/WGS/variant_calls/GTEx_Analysis_2017-06-05_v8_WholeGenomeSeq_support_files/GTEx_Analysis_2017-06-05_v8_WholeGenomeSeq_838Indiv_Analysis_Freeze_20genotPCs.txt', sep='\t', header=T)
ax = fread('gtex-admixed0.9.txt',sep='\t',header=F)
...
# fit a model for each gene and extract R-sq
