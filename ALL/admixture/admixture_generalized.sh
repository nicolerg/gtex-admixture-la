...
outdir=/mnt/lab_data/montgomery/nicolerg/gtex-tg-merged/admixture_20190819-3
tg_meta=/users/nicolerg/gtex-admix/metadata/1kg-subject-popcodes.tsv
admix=/mnt/lab_data/montgomery/nicolerg/admixed-ancestry/gtex-admixed0.9.txt

# run admixture on 117 GTEx individuals with any set of reference panels (equal numbers from each 1000 Genomes superpop requested)

cd ${outdir}

# select reference populations 
vcf_header=../genotyped_subjects.txt
zgrep -m1 -E "^#CHROM" ../merged_hg38_chr16.vcf.gz | cut --complement -f1-9 | sed "s/	/\n/g" > ${vcf_header}

# string with user-defined SUPERPOPULATIONS
Rscript ~/gtex-admix/scripts/admixture/select_reference_populations.R 'EUR,EAS,AFR,SAS' ${vcf_header} ${tg_meta} ${admix}
K=4

# filter subjects and re-apply MAF filter
cut -f1 keep_subjects.txt > bcftools_keep.txt
for chr in {1..22}; do 
	bcftools view --force-samples -S bcftools_keep.txt -Oz --min-af 0.1 -o merged_hg38_chr${chr}_filt_MAF10.vcf.gz ../merged_hg38_chr${chr}.vcf.gz &
done 
wait

# remove SNPs with any missing values 
for chr in {1..22}; do 
	bcftools view -g ^miss -Oz -o merged_hg38_chr${chr}_filt_MAF10_nomissing.vcf.gz merged_hg38_chr${chr}_filt_MAF10.vcf.gz &
done
wait
...
for chr in {1..22}; do 
	plink --vcf merged_hg38_chr${chr}_filt_MAF10_nomissing.vcf.gz --make-bed --vcf-half-call m --out merged_hg38_chr${chr}_filt_MAF10_nomissing & 
done
wait
# LD-prune each chromosome 
for chr in {1..22}; do 
	plink --bfile merged_hg38_chr${chr}_filt_MAF10_nomissing --indep-pairwise 40 10 0.1 --out merged_hg38_chr${chr}_filt_MAF10_nomissing &
done
wait
for chr in {1..22}; do 
	plink --bfile merged_hg38_chr${chr}_filt_MAF10_nomissing --extract merged_hg38_chr${chr}_filt_MAF10_nomissing.prune.in --make-bed --out merged_hg38_chr${chr}_filt_MAF10_nomissing_pruned &
done
wait
...
Rscript ~/gtex-admix/scripts/admixture/order_pop_file.R merged_hg38_filt_MAF10_nomissing_pruned.fam ${tg} 
...
#cp merged_hg38_filt_MAF10_nomissing_pruned.3.Q.subj ~/gtex-admix/admixture/merged_hg38_filt_MAF10_nomissing_pruned_mixed-reference-panels.3.Q.subj
cp merged_hg38_filt_MAF10_nomissing_pruned.${K}.Q.subj ~/gtex-admix/admixture/merged_hg38_filt_MAF10_nomissing_pruned_mixed-${K}-reference-panels.${K}.Q.subj
