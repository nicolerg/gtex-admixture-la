...
geno <- paste0("/mnt/lab_data/montgomery/nicolerg/genotypes-hg19/gtex-phased-MAF0-hg19/gtex.chr",chr,".vcf.gz")
expr <- "/mnt/lab_data/montgomery/shared/datasets/gtex/GTEx_Analysis_2017-06-05_v8/eqtl/GTEx_Analysis_v8_eQTL_expression_matrices/Cells_EBV-transformed_lymphocytes.v8.normalized_expression.bed.gz"
globalcov <- "/mnt/lab_data/montgomery/shared/datasets/gtex/GTEx_Analysis_2017-06-05_v8/eqtl/GTEx_Analysis_v8_eQTL_covariates/Cells_EBV-transformed_lymphocytes.v8.covariates.txt"
...
# only keep as many white subjects as there are admixed
...
white <- pheno[ RACE == '3' , SUBJID ][1:nadx]
# filter expression file
subjs <- c(pheno[ RACE == '1' | RACE == '2', SUBJID ], white)
...
# remove "EUR" from local cov file
localcov[,EUR:=NULL]
...
system("echo")
localwhite <- data.table(SUBJID=white, 
	AFR=rep(0, length(white)), 
	ASN=rep(0, length(white)), 
	UNK=rep(0, length(white)))
