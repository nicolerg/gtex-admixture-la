...
outdir="/mnt/lab_data/montgomery/nicolerg/local-eqtl/lung"
indir="${outdir}/chunks"

for chr in {1..22}
do	
	geno="/mnt/lab_data/montgomery/nicolerg/filtered/gtex.MAF01.phased.chr${chr}.vcf.gz"
	localcov="/mnt/lab_data/montgomery/nicolerg/local-eqtl/chr${chr}.hg19.localcov.tsv"
	...
	do
		...
		out2="${outdir}/lung-local-unkcnt-chr${chr}-${suf}.tsv"
		taskset -c 43-47 Rscript slow-eqtl-local-optimized.R ${geno} ${gene} ${globalcov} ${localcov} ${pheno} ${out1} ${out2} &
