...
expr <- paste0("/mnt/lab_data/montgomery/shared/datasets/gtex/GTEx_Analysis_2017-06-05_v8/eqtl/GTEx_Analysis_v8_eQTL_expression_matrices/",tissue,".v8.normalized_expression.bed.gz")
...
# filter localcov file
localcov[,EUR:=NULL]
strTmp = c('AFR','ASN','UNK') # Name of columns to be converted to numeric
localcov[, (strTmp) := lapply(.SD, as.integer), .SDcols = strTmp]
nrow(localcov)
localcov <- localcov[ UNK == 0]
nrow(localcov)
localcov[,UNK := NULL]
...
for(j in 1:nrow(geno)){
	...
	for(i in 1:nrow(expr)){
		...
		if(tss_distance <= 1000000 & tss_distance >= -1000000){
			...
			df$SNP_ID <- NULL
			df$POS <- NULL
			if(length(unique(df$platform))==1){
				df$platform <- NULL
			}
