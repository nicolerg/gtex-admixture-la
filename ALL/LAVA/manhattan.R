...
#chr21_pairs <- paste(indir,'/wholeblood-local-allpairs-chr21.tsv.gz',sep='')
...
asn_sig <- unique(allpairs_asn[1:10000,1])[1:2]
asn_sig
...
jpeg(paste(outdir,'/manhattan-wholeblood-afr-siggenes.jpg',sep=''), width=1000, height=600, res=150)
ggplot(allpairs_afr) + 
...
# 	geom_point(aes(x=pos,y=pval_nominal), colour="black", size=0.5, alpha=0.4) + 
# 	geom_point(aes(x=pos,y=s), colour="red", size=0.5, alpha=1) + 
# 	labs(y='Nominal P-Value (-log10)', x="Position on chr21", title='Significance of Genotype Dosage') +
# 	theme(axis.ticks.x = element_blank(),
#       axis.text.x = element_blank(),
#       plot.title = element_text(hjust = 0.5),
#       legend.position="none") +
