...
eur <- allpeur[,pval_nominal < cutoff]
afr <- allpafr[,pval_nominal < cutoff]

mergeallp <- merge(afr, eur, by=c("gene_id","variant","tss_distance"), suffixes = c(".AFR",".EUR"))
...
mergeallp[,sig_eur:=ifelse(pval_nominal.EUR < strict,1,0)]
mergeallp[,sig_afr:=ifelse(pval_nominal.AFR < strict,1,0)]
mergeallp[,sig:=ifelse(sig_eur==1 & sig_afr==1,1,0)]
head(mergeallp)

notsig <- mergeallp[sig==0]
sig <- mergeallp[sig==1]
sig_eur <- mergeallp[sig_eur==1]
sig_afr <- mergeallp[sig_afr==1]
...
ggplot(data=NULL, aes(x=slope.EUR, y=slope.AFR)) +
	geom_point(data=notsig,size=0.6,alpha=0.1,colour="gene-level significance") +
	geom_point(data=sig_eur,size=0.6,alpha=0.5,colour="EUR eQTL") +
	geom_point(data=sig_afr,size=0.6,alpha=0.5,colour="AFR eQTL") +
	geom_point(data=sig,size=0.6,alpha=0.8,colour="eQTL in both") +
	theme_bw() +
	labs(x="EUR gene-SNP pair effect size",y="AFR gene-SNP pair effect size") +
	scale_colour_manual(values=c("gene-level significance"="black",
		"EUR eQTL"="blue",
		"AFR eQTL"="orange",
		"eQTL in both"="red")) 
dev.off()
