...
### DEPRECATED. all plots have been added to august
...
# # add some minimum difference 
# m_sub = m_sub[(coloc_sig & abs(coloc_h4_global-coloc_h4_local) > 0.1) | (finemap_sig & (abs(finemap_clpp_global-finemap_clpp_local) > 0.01))]
# m_sub
# table(m_sub[coloc_sig == T, coloc_global_better])
# table(m_sub[finemap_sig == T, finemap_global_better])
# m_sub2 = m_sub[coloc_sig == T & finemap_sig == T] # 28 
# table(m_sub2[,coloc_global_better], m_sub2[,finemap_global_better])
...
grid.arrange(COLOC(), FINEMAP(), layout_matrix=rbind(c(1,1,2,2),c(1,1,2,2)))
dev.off()
