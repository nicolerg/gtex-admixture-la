...
# COLOC eQTL calling pipeline for Global and Local results; all tissues; same eGene, different lead SNP at 1e-04

srcdir=~/gtex-admixture-la/coloc

indir=local-eqtl/admixed
base=${indir}/coloc

# list of same egenes, diff lead SNP output by get_uniq_egenes.R 
gene_list=same_egene_diff_lead_1e-04.txt

# get AF of every SNP tested in admixed individuals (AF only within admixed population)
for chr in {1..22}; do 
	vcf=geno/gtex.admixed.MAC10.phased.chr${chr}.recode.vcf.gz
...
for tissue in Skin_Not_Sun_Exposed_Suprapubic Muscle_Skeletal Nerve_Tibial Artery_Tibial Lung Adipose_Subcutaneous; do 
	python ${srcdir}/parse_allpairs_for_coloc.py ${tissue} ${gene_list} ${indir} &
done
wait
...
for file in `ls ${base}/allpairs_difflead | grep "allpairs.tsv"`; do # Skin_Not_Sun_Exposed_Suprapubic.filtered.local.allpairs.tsv.gz
	new_file=$(echo $file | sed "s/allpairs\.tsv/allpairs\.sorted\.tsv/")
...
wait
for file in `ls ${base}/allpairs_difflead | grep "allpairs.sorted.tsv.gz"`; do
...
# scatterplot with annotation/compile_coloc.R
...
Rscript ${srcdir}/compile_coloc.R ${colocdir}
