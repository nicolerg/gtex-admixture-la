...
for (gene in expr[,gene_id]){
	...
	# access with permuted_expr[[gene]][[i]]
}
...
dt = data.table(gene_id = expr[,gene_id], 
				tissue = Tissue,
...
for (gene in names(minimum_pvalues)){
	for(m in c('global','LAVA')){
		# get p-value vector
		x = minimum_pvalues[[gene]][[m]]
		x = x[!is.na(x)]
		# get beta parameters from ML
		ml = fitdist(x, "beta", method="mle")
...
outfile = sprintf('%s.beta_est.RData',gsub('\\..*', '',out))
save(beta_adjusted, file=outfile)
