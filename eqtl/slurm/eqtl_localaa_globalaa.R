...
#geno <- paste0("/labs/smontgom/nicolerg/LAVA/admixed_vcf/gtex.admixed.MAC10.phased.chr",chr,".recode.vcf.gz")
#localcov <- paste0("/labs/smontgom/nicolerg/LAVA/admixed_ancestry/chr",chr,".hg19.maf0.localcov.tsv.gz")
keep <- "/labs/smontgom/nicolerg/src/gtex-admixture-la/gtex-admixed0.9.txt"

writeLines(sprintf("Starting chr%s, %s",chr,exprfile))
...
# PEER factors already limited to first 15 in admixed individuals
...
if(nrow(geno) == 0){
    system("echo 'No variants are within 1MB of any TSSs in this gene list. Aborting.'")
    q()
}
info_col <- geno[,1:9]
...
allp <- file(out, 'w')
cat("gene_id\tvariant_id\ttss_distance\tma_samples\tma_count\tglobal_pval_nominal\tglobal_slope\tglobal_slope_se\tlava_pval_nominal\tlava_slope\tlava_slope_se\tafr_pval_nominal\tafr_slope\tafr_slope_se\n", file=allp)
...
for(j in 1:nrow(geno)){
    ...
    for(i in 1:nrow(expr)){
        ...
        if(tss_distance <= 1000000 & tss_distance >= -1000000){
            ...
            string=paste(gene_id,variant_id,tss_distance,ma_samples,ma_count,global_pval_nominal,global_slope,global_slope_se,lava_pval_nominal,lava_slope,lava_slope_se,afr_pval_nominal,afr_slope,afr_slope_se, sep='\t')
